/* rpd - a command-line music player
Copyright (C) 2022 Ravachol

http://github.com/ravachol/rpd
...
void cleanupOnExit()
{
        ...
        if (noMusicFound)
        {
                ...
                printf("To set it type: rpd path \"/path/to/Music\". \n");
        }
        else if (noPlaylist)
        {
...
// Ensures only a single instance of rpd can run at a time for the current user.
void exitIfAlreadyRunning()
{
        char pidfile_path[256];

        snprintf(pidfile_path, sizeof(pidfile_path), "%s%s", TMPFOLDER, "/rpd_");
        ...
        if (pidfile != NULL)
        {
                if (fscanf(pidfile, "%d", &pid) == 1)
                {
                        ...
                        {
                                fprintf(stderr, "An instance of rpd is already running. Pid: %d.\n", pid);
                                exit(EXIT_FAILURE);
                        }
                        else
                        {
